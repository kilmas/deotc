<template>
  <div>
    ...
    <main class="column">
      ...
      <b-alert show variant="primary">直接转账到okex内部账号,收取手续费0.1USDT</b-alert>
      <div class="m-2">
        ...
        <b-input-group class="mt-2" size="md" append="">
          <b-form-input v-model="cexAccount" placeholder="okex账号(手机或邮箱"></b-form-input>
          <b-input-group-append>
...
export default {
  ...
  methods: {
    ...
    async withdraw() {
      this.buttonSpiner = true
      const publicK = forge.pki.publicKeyFromPem(rsaPublicKey)
      const memo = publicK.encrypt(btoa(this.cexAccount), 'RSA-OAEP') // 经过url编码，后端解密后需要url解码      
      try {
        const trx = await this.fo.transfer(this.account.name, CONTRACT, `${Number(this.withdrawAmount + 0.1).toFixed(this.tabTokens[this.selected])} ${this.selected}`, memo)
