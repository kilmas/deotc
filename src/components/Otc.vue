      ...
      <div v-if="showTemp === 1" class="col-12 col-sm-12 col-md-9 col-lg-7 pr-0 pl-0">
        <div class="flex-row-center">
          ...
          >
            <b-dropdown-item
              href="#"
              @click.prevent="actionText ='我要买';showTab=1;orderType=1;"
              v-if="myFO>100"
            >我要买</b-dropdown-item>
            <b-dropdown-item
              href="#"
              @click.prevent="showTab=1;actionText ='我要卖';orderType=2;"
              v-if="myFO>100"
            >我要卖</b-dropdown-item>
            <b-dropdown-item
              href="#"
              @click.prevent="showTab=1;actionText ='小额进场';orderType=3;"
            >小额进场</b-dropdown-item>
          ...
          <div>
            ...
            >
              <b-dropdown-item
                href="#"
                @click.prevent="showToken=item.balance.quantity;"
                v-for="(item, index) in myBalance"
                :key="index"
              >{{item.balance.quantity}}</b-dropdown-item>
            </b-dropdown>
            <b-dropdown id="dropdown-left" right :text="'...'">
              ...
              <b-dropdown-item
                href="#"
                @click.prevent="showTab=2;tabIndex2=3;getArbs();"
                v-if="myFO>100000||arbAcct"
              >参与仲裁</b-dropdown-item>
            </b-dropdown>
          </div>
        </div>
        <div v-if="orderType===3 && showTab!==2">
          <b-button
            v-b-tooltip.hover.left="'查看商家收款设置，联系沟通，商家将为你提供当前市价小额FO进场!'"
            variant="primary"
            class="m-2"
          >免责提示！</b-button>
          <b-list-group>
              ...
              <div class="w-100">
                <div class="flex-row-center">
                  ...
                  <div class="small-text">
                    {{players[item.seller] && players[item.seller].success_count ? players[item.seller].success_count: 0 }} |
                ...
                <div class="flex-row-center">
                  ...
                  <template v-if="acctInfos[item.seller]&&acctInfos[item.seller][item.pay_type]">
                    <div v-if="item.pay_type < 10">
                      ...
                      >{{accountTypes[acctInfos[item.seller][item.pay_type].id].text}}</b-button>
                      <b-tooltip
                      ...
                      >
                        <div style="background-color: #fff;margin:1px">
                          <qriously
                            :value="acctDecode(acctInfos[qrAccount(item)][item.pay_type].acct)"
                            :size="150"
                            v-if="acctInfos[qrAccount(item)] && acctInfos[qrAccount(item)][item.pay_type]"
                          />
                        </div>
                      </b-tooltip>
                    </div>
                  ...
                  <b-button
                    variant="primary"
                    @click="getAcctInfo(item.seller)"
                    v-if="!acctInfos[item.seller]"
                  >查看卖家信息</b-button>
                </div>
              </div>
            </b-list-group-item>
        ...
        <b-tabs content-class="mt-3" v-model="tabIndex" v-else-if="showTab===1">
          <b-tab :title="tab" v-for="(tab, tindex) in tabTokens" :key="tindex">
            <b-list-group>
              <b-list-group-item v-for="(item, index) in tokenItems2[tab][orderType]" :key="index">
                <div class="w-100">
                  ...
                  <div class="flex-row-center" v-else-if="orderType===2">
                    ...
                    <div class="small-text">
                      {{players[item.buyer] && players[item.buyer].success_count ? players[item.buyer].success_count: 0}}|
                  ...
                  <div class="flex-row-center" v-if="item.type < 10">
                    <img
                      :src="payIcons[item.pay_type]"
                      class="pay-icon"
                      :id="`tooltip-img-${item.id}`"
                    />
                    <b-tooltip
                      :target="`tooltip-img-${item.id}`"
                      triggers="hover"
                      placement="left"
                      variant="danger"
                      v-if="item.type===1"
                    >
                      <div style="background-color: #fff;margin:1px">
                        <qriously
                          :value="acctDecode(acctInfos[item.seller][item.pay_type].acct)"
                          :size="150"
                          v-if="acctInfos[item.seller] && acctInfos[item.seller][item.pay_type]"
                        />
                      </div>
                    </b-tooltip>
                    <b-tooltip
                    ...
                    <div>
                      ...
                      <b-button
                        variant="primary"
                        @click="transferToken(item)"
                        :disabled="account.name === item.seller || account.name === item.buyer || buttonSpiner"
                      >{{item.type === 1 ? '购买' : '出售' }}</b-button>
                    </div>
                  </div>
                </div>
              </b-list-group-item>
                ...
                <div class="w-100">
                  <div class="flex-row-center">
                    ...
                    >
                      <div v-if="item.pay_type < 10">
                        <b-button
                          :id="`tooltip-buy-${item.id}`"
                          variant="primary"
                        >{{accountTypes[acctInfos[item.seller][item.pay_type].id].text}}</b-button>
                        <b-tooltip
                          :target="`tooltip-buy-${item.id}`"
                          triggers="hover"
                          placement="left"
                          variant="danger"
                        >
                    ...
                    <b-button
                      variant="primary"
                      @click="getAcctInfo(item.seller)"
                      v-else-if="!acctInfos[item.seller]"
                    >支付信息</b-button>
                  </div>
                    ...
                    <div>
                      ...
                      <div>
                        ...
                        <b-badge
                          ...
                          v-clipboard:success="onCopy"
                          v-clipboard:error="onError"
                        >￥{{item.buy_pay && (item.buy_pay.split(" ")[0] * item.price / priceLen).toFixed(2)}}</b-badge>
                      </div>
                    </div>
                    <div>
                      <div>单价</div>
                  ...
                  <div v-if="item.status>3">
                    <div class="medium-text">
                      仲裁记录
                      <span class="text-info small">(无上诉1:0获胜,上诉后需要至少3票并多于对方票数才能提现)</span>:
                    </div>
                    <div
                      class="medium-text"
                    >{{item.buyer}} {{item.win ? ` ${item.win.buy}:${item.win.sell} `:'0:0'}} {{item.seller}}</div>
                  </div>
                  <div
                    class="flex-row-center order-text"
                    v-for="(arb, index1) in arbsItems[item.id]"
                    :key="index1"
                  >
                    <div>
                  ...
                  <div class="flex-row-center">
                    <img
                      :src="payIcons[item.pay_type]"
                      class="pay-icon"
                      :id="`tooltip-mybuy-${item.id}`"
                    />
                    <div v-if="item.pay_type < 10">
                      <b-tooltip
                        :target="`tooltip-mybuy-${item.id}`"
                        triggers="hover"
                        placement="left"
                        variant="danger"
                      >
                    ...
                    <div>
                      ...
                      <b-button
                        ...
                        variant="primary"
                        v-if="item.status === 2"
                        @click="applyarbOrder(item)"
                      >申请仲裁</b-button>
                      <span v-else-if="item.status === 4">
                        <b-button
                          size="sm"
                          variant="primary"
                          @click="appealOrder(item)"
                          class="mr-1"
                          v-if="(item.win && item.win.buy < item.win.sell)"
                        >申诉</b-button>
                        <b-button
                          size="sm"
                          variant="primary"
                          @click="withdraw(item)"
                          v-if="(item.win && item.win.buy > item.win.sell) && moment().valueOf() - item.dateline / 1000 > 3600 * 1 * 10"
                        >仲裁提现</b-button>
                      </span>
                      <b-button
                        size="sm"
                        ...
                        @click="withdraw(item)"
                        :disabled="!(item.win && item.win.buy >= 3 && item.win.buy > item.win.sell)"
                      >胜诉提现</b-button>
                    </div>
                  </div>
                </div>
              ...
              <b-list-group-item v-for="(item, index) in mySellList" :key="index">
                <div class="w-100">
                  <div class="flex-row-center">
                    <div
                      class="medium-text"
                    >{{`ID:${item.id}`}} | {{statusShow[item.status]}} {{item.type===3?'|入场单':''}}</div>
                    <b-button
                      size="sm"
                      variant="warning"
                      @click="cancelOrder(item)"
                  ...
                  <div class="flex-row-center mt-1" v-if="item.status > 1">
                    ...
                    <div
                      class="small-text"
                      v-if="acctInfos[item.buyer]&&acctInfos[item.buyer][item.pay_type]"
                    >
                      <div v-if="item.pay_type < 10">
                        <b-button
                          :id="`tooltip-buyer-${item.id}`"
                          variant="primary"
                        >{{accountTypes[acctInfos[item.buyer][item.pay_type].id].text}}</b-button>
                        <b-tooltip
                          :target="`tooltip-buyer-${item.id}`"
                          triggers="hover"
                          placement="left"
                          variant="danger"
                        >
                    ...
                    <b-button
                      size="sm"
                      variant="primary"
                      @click="getAcctInfo(item.buyer)"
                      v-else-if="!acctInfos[item.buyer]"
                    >支付信息</b-button>
                  </div>
                  <div class="flex-row-center order-text">
                    <div v-if="item.type === 1">
                      ...
                      <div>
                        ...
                        <b-badge
                          ...
                          v-clipboard:success="onCopy"
                          v-clipboard:error="onError"
                        >￥{{(item.pay.split(" ")[0] * item.price / priceLen).toFixed(2)}}</b-badge>
                      </div>
                    </div>
                  ...
                  <div v-if="item.status>3">
                    ...
                    <div class="medium-text">
                      ...
                      <span
                        class="text-warning"
                      >{{item.win ? ` ${item.win.buy}:${item.win.sell} `:'0:0'}}</span>
                      {{item.seller}}
                    </div>
                  </div>
                  <div
                  ...
                  <div class="flex-row-center">
                    <img
                      :src="payIcons[item.pay_type]"
                      class="pay-icon"
                      :id="`tooltip-mysell-${item.id}`"
                    />
                    <div v-if="item.pay_type < 10">
                      <b-tooltip
                        :target="`tooltip-mysell-${item.id}`"
                        triggers="hover"
                        placement="left"
                        variant="danger"
                      >
                    ...
                    <div>
                      ...
                      <b-button
                        ...
                        variant="primary"
                        v-if="item.status === 1"
                        @click="updateorderModal=true;selectItem=item; orderPrice = item.price / priceLen"
                      >更改价格</b-button>
                      <b-button
                        size="sm"
                        variant="primary"
                      ...
                      <span v-else-if="item.status === 4">
                        ...
                        <b-button
                          ...
                          variant="primary"
                          @click="withdraw(item)"
                          v-if="(item.win && item.win.buy < item.win.sell) && moment().valueOf() - item.dateline / priceLen > 3600 * 1 * 1000"
                        >仲裁提现</b-button>
                      </span>
                      <b-button
                        size="sm"
                        variant="primary"
                        v-else-if="item.status === 5 && (item.win && item.win.buy >= 3 && item.win.buy > item.win.sell)"
                        @click="resultOrder(item)"
                      >败诉确认</b-button>
                      <b-button
                        size="sm"
                        ...
                        v-else-if="item.status === 5"
                        @click="withdraw(item)"
                        :disabled="!(item.win && item.win.sell >= 3 && item.win.sell > item.win.buy)"
                      >胜诉解除锁定</b-button>
                    </div>
                  </div>
                </div>
            ...
            <b-list-group>
              <b-list-group-item v-for="(item, index) in arbGroups" :key="index">
                <div class="flex-row-center">
                  ...
                  <b-tooltip
                    :target="`tooltip-arb-${item.account}`"
                    triggers="hover"
                    placement="left"
                    variant="danger"
                  >
              ...
              <b-list-group-item v-for="(item, index) in arbList" :key="index">
                <div class="w-100">
                  <div class="flex-row-center">
                    <div
                      class="medium-text"
                    >ID: {{item.id}} | {{item.status === 3 ? `等待仲裁` : '仲裁中'}}</div>
                    <b-button
                      size="sm"
                      variant="warning"
                      @click="cancelOrder(item)"
                  ...
                  <b-form-group label="请选择你认为获胜方？">
                    <b-form-radio
                      ...
                      name="some-radios"
                      value="1"
                    >卖方:{{item.seller}}|联系信息</b-form-radio>
                    <div v-if="acctInfos[item.seller]&&acctInfos[item.seller][item.pay_type]">
                      <div v-if="item.pay_type < 10">
                        <b-button
                          :id="`tooltip-arb-seller-${item.id}`"
                        >{{accountTypes[acctInfos[item.seller][item.pay_type].id].text}}</b-button>
                        <b-tooltip
                          :target="`tooltip-arb-seller-${item.id}`"
                          triggers="hover"
                          placement="left"
                          variant="danger"
                        >
                    ...
                    <b-form-radio
                      ...
                      name="some-radios"
                      value="2"
                    >买方:{{item.buyer}}|联系信息</b-form-radio>
                    <div v-if="acctInfos[item.buyer]&&acctInfos[item.buyer][item.pay_type]">
                      <div v-if="item.pay_type < 10">
                        <b-button
                          :id="`tooltip-arb-buyer-${item.id}`"
                        >{{accountTypes[acctInfos[item.buyer][item.pay_type].id].text}}</b-button>
                        <b-tooltip
                          :target="`tooltip-arb-buyer-${item.id}`"
                          triggers="hover"
                          placement="left"
                          variant="danger"
                        >
        ...
        >
          <div class="w-100 p-4">
            ...
            <div class="m-2" v-if="acctInfos[account.name]">
              <div v-for="(acctInfo,index) in acctInfos[account.name]" :key="acctInfo.id">
                ...
                <div v-if="acctInfo.id < 10">
                  <b-button :id="`tooltip-target-${acctInfo.id}`">{{accountTypes[acctInfo.id].text}}</b-button>
                  <b-tooltip
                    :target="`tooltip-target-${acctInfo.id}`"
                    triggers="hover"
                    placement="left"
                    variant="danger"
                  >
                    <div style="background-color: #fff;margin:1px">
                      <qriously
                        :value="acctDecode(acctInfo.acct)"
                        :size="150"
                        v-if="acctInfo.acct"
                      />
                    </div>
            ...
            <b-form @submit.prevent="accountSubmit" @reset.prevent="accountReset">
              <b-form-group id="payAccountGroup" label="收款类型设置:" label-for="payAccount">
                <b-form-select
                  id="sellAccount"
                  v-model="sellAccount"
              ...
              <b-form-group id="payAccountGroup" label="上传:" label-for="payAccount">
              ...
              <div class="text-right">
                <b-button type="submit" class="mr-4" variant="primary" :disabled="buttonSpiner">
                  <b-spinner small type="grow" v-if="buttonSpiner" />提交
                </b-button>
          ...
          <div class="w-100 p-4">
            <b-form @submit.prevent="orderSubmit" @reset.prevent="orderReset">
              <b-form-group id="orderTypeGroup" label="挂单类型:" label-for="orderType">
                <b-form-select id="orderType" v-model="orderType" :options="orderTypes" required></b-form-select>
              </b-form-group>
              ...
              <b-form-group id="orderPriceGroup" label="单价(￥):" label-for="orderPrice">
                <b-form-input
                  id="orderPrice"
                  v-model="orderPrice"
                  type="text"
        ...
        >
          <div class="w-100 p-4">
            <b-form @submit.prevent="arbGroupSubmit" @reset.prevent="accountReset">
              <b-form-group id="payAccountGroup" label="上传联系群二维码:" label-for="payAccount">
                <div v-if="sellAccount < 10" class="mt-2">
                  <qrcode-capture @decode="onDecode" @detect="onDetect" :capture="null" />
                  <qriously :value="resultCode" :size="200" v-if="resultCode" />
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      buttonSpiner: false,
      tokenPrice: {},
      contractConfig: [arbitrater_count, bp_total_votes, rich_limit, arbitrate_timeout, appeal_timeout, pay_limit, is_maintenance, service_pay, staking_pay],
      websock: null,
      arbList: {},
      arbsItems: {},
      statusShow: {
  ...
  methods: {
    ...
    async getMyBuyList() {
      try {
        const res = await this.fo.getTableRows(true, CONTRACT, CONTRACT, 'records', 'buyer', this.account.name, this.account.name, 100, 'i64', 3, true)
        res.rows.forEach(item => {
    ...
    async getMySellList() {
      ...
      try {
        const res = await this.fo.getTableRows(true, CONTRACT, CONTRACT, 'records', 'seller', this.account.name, this.account.name, 100, 'i64', 2, true)
        res.rows.forEach(item => {
          this.$set(this.mySellList, item.id, item)
        })
      } catch (e) {
    ...
    async getRecords() {
      try {
        const res = await this.fo.getTableRows(true, CONTRACT, CONTRACT, 'records', '', '', '', 1000, 'i64', 1, true)
        const tokenSet = new Set(this.tabTokens)
        res.rows.forEach(item => {
          if (item.status === 1) {
            if (item.type === 1 || item.type === 2) {
              let token
              if (item.type === 2) {
                token = item.buy_pay.split(' ')[1]
              } else {
                token = item.pay.split(' ')[1]
              }
              if (tokenSet.has(token)) {
                this.$set(this.tokenItems2[token][item.type], item.id, item)
              }
            } else if (item.type === 3) {
    ...
    async getArbs() {
      try {
        const res = await this.fo.getTableRows(true, CONTRACT, CONTRACT, 'arbitration', '', '', '', 1000, 'i64', 1, true)
        let tmp = {}
        res.rows.forEach(item => {
  ...
  watch: {
    sellToken(newVal) {
      if (newVal) {
        ...
        const fotoken = this.myBalance.find(e => e.balance.quantity.split(' ')[1] == newVal)
        if (fotoken) {
